...
void NetworkActionsComponent::onInitialize() {
    ...
    tcpMessages = addStringParam("tcp_messages", 0, 1);
    httpMessages = addStringParam("http_messages", 0, 1);
    wsMessages = addStringParam("ws_messages", 0, 1);
...
void NetworkActionsComponent::wifi_event_callback(bool connected, void* user_data) {
    ...
    // Update wifi_connected parameter using member pointer
    if (self->wifiConnected) {
        self->wifiConnected->setValue(0, 0, connected);
    }
...
void NetworkActionsComponent::loadAllMessageExamples() {
    ...
    // Use member pointers (they should already be assigned)
    if (!tcpMessages || !httpMessages || !wsMessages) {
    ...
    for (size_t i = 0; i < MessageExamples::ALL_EXAMPLES_COUNT; i++) {
        ...
        if (host) {
            tcpMessages->appendValue(std::string(item_json));
            tcp_count++;
        } else if (message) {
            wsMessages->appendValue(std::string(item_json));
            ws_count++;
        } else if (url) {
            httpMessages->appendValue(std::string(item_json));
            http_count++;
        }
...
bool NetworkActionsComponent::parseTcpMessageAt(size_t index, TcpMessage& out) const {
    if (!tcpMessages || index >= tcpMessages->getRows()) {
        return false;
    }
    
    const std::string& json_str = tcpMessages->getValue(index, 0);
...
bool NetworkActionsComponent::parseHttpMessageAt(size_t index, HttpMessage& out) const {
    if (!httpMessages || index >= httpMessages->getRows()) {
        return false;
    }
    
    const std::string& json_str = httpMessages->getValue(index, 0);
    if (json_str.empty()) return false;
...
bool NetworkActionsComponent::parseWsMessageAt(size_t index, WsMessage& out) const {
    if (!wsMessages || index >= wsMessages->getRows()) {
        return false;
    }
    
    const std::string& json_str = wsMessages->getValue(index, 0);
    if (json_str.empty()) return false;
...
std::string NetworkActionsComponent::getTcpMessageName(size_t index) const {
    if (!tcpMessages || index >= tcpMessages->getRows()) return "";
...
std::string NetworkActionsComponent::getHttpMessageName(size_t index) const {
    if (!httpMessages || index >= httpMessages->getRows()) return "";
...
std::string NetworkActionsComponent::getWsMessageName(size_t index) const {
    if (!wsMessages || index >= wsMessages->getRows()) return "";
